...
for (i in seq(from = 1, to = 5, by = 1)) {
  ...
  for (j in seq(from = 0.6, to = 0.9, by = 0.1)) {
    ...
    predicted <- predict(modelRes, dataTrait.test) #factor of predicted traits
    ...
    prediction <- tibble(predicted$predictions) %>% 
    ...
    confusionMatrix <- confusionMatrix(dataTrait.test$Trait, predicted$predictions, mode = "everything") #confusion matrix
...
write.xlsx(accuracy, file = "Trait_ranger_accuracy.xlsx")
write.xlsx(comparison, file = "Trait_ranger_comparison.xlsx")
write.xlsx(confusionMatrix, file = "Trait_ranger_confusionMatrix.xlsx")
