...
for (i in seq(from = 1, to = 5, by = 1)) {
  ...
  for (j in seq(from = 0.6, to = 0.9, by = 0.1)) {
    ...
    predicted <- predict(modelRes, dataClass.test) #factor of predicted classes
    ...
    confusionMatrix <- confusionMatrix(dataClass.test$Class, predicted$predictions, mode = "everything") #confusion matrix
...
write.xlsx(accuracy, file = "Gene_ranger_accuracy.xlsx")
write.xlsx(comparison, file = "Gene_ranger_comparison.xlsx")
write.xlsx(confusionMatrix, file = "Gene_ranger_confusionMatrix.xlsx")
